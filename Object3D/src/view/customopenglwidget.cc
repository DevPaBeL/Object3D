...
//инициализация и проверка глубины
...
pav::Vector3 CustomOpenGLWidget::getObjectTranslate() {
  return object_translate_;
}

pav::Vector3 CustomOpenGLWidget::getObjectRotation() {
  return object_rotation_;
}

pav::ObjectController& CustomOpenGLWidget::getController() {
  return object_;
}
...
void CustomOpenGLWidget::paintSolidEdges() {
  ...
  std::vector<pav::Vector3> vertices = object_.GetVertices();
  std::vector<std::pair<unsigned, unsigned>> edges = object_.GetEdges();

  glBegin(GL_LINES);
  glColor3f(edge_color_.red, edge_color_.green, edge_color_.blue);
  for (const auto& edge : edges) {
    const pav::Vector3& vertex1 = vertices[edge.first];
    const pav::Vector3& vertex2 = vertices[edge.second];
...
void CustomOpenGLWidget::paintDashedEdges() {
  ...
  for (const auto& edge : edges) {
    const pav::Vector3& v1 = vertices[edge.first];
    const pav::Vector3& v2 = vertices[edge.second];
